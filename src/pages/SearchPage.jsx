...
const exactMatch = (post, searchKeyString, searchIn) => {
  ...
  if (searchIn.length === 2) {
    return (
      postTitle.includes(searchKeyLower) || postContent.includes(searchKeyLower)
    );
  }
...
const SearchPage = () => {
  ...
  const [searchIn, setSearchIn] = useState(
    location.state?.in ?? ["Title", "Content"]
  );
  ...
  const filteredPosts = useMemo(() => {
    ...
    if (
      searchKeyString !== '""' &&
      searchKeyString.startsWith('"') &&
      searchKeyString.endsWith('"')
    ) {
      const trimmedSearchKey = searchKeyString.slice(1, -1);
      filtered = filtered.filter((post) =>
        exactMatch(post, trimmedSearchKey, searchIn)
      );
    } // Fuzzy match
    else {
